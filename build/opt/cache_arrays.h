...
class CacheArray : public GlobAlloc {
    public:
	 ...
	 inline uint32_t getCompressedSizeFromAddr(Address addr) {
            // return blockSize;
            char buffer[ 64 ];
            PIN_SafeCopy(buffer, (void*)(addr << lineBits), 64);
            ...
            uint32_t size = BDICompress(buffer, 64);
            //info("BDICompress: addr %lx size %u", addr << 6, size);
            return size;
            //return BDICompress(buffer, blockSize);
            //return GeneralCompress(buffer, blockSize, 3);
        }
	 inline void getBlock(char* block, Address addr) {
		 PIN_SafeCopy(block, (void*)(addr << lineBits), 64);
	 }
