...
class CC : public GlobAlloc {
    public:
        ...
        virtual uint64_t processEviction(const MemReq& triggerReq, Address wbLineAddr, int32_t lineId, uint64_t startCycle, bool* downgrade) = 0; //called iff shouldAllocate returns true
...
class MESIBottomCC : public GlobAlloc {
    ...
    public:
        ...
        uint64_t processEviction(Address wbLineAddr, uint32_t lineId, bool lowerLevelWriteback, uint64_t cycle, uint32_t srcId, bool* downgrade);
...
class MESITopCC : public GlobAlloc {
    ...
    public:
        ...
        uint64_t processEviction(Address wbLineAddr, uint32_t lineId, bool* reqWriteback, uint64_t cycle, uint32_t srcId, bool* downgrade);
    ...
    private:
        uint64_t sendInvalidates(Address lineAddr, uint32_t lineId, InvType type, bool* reqWriteback, uint64_t cycle, uint32_t srcId, bool* downgrade);
};
    ...
    public:
        ...
        bool shouldAllocate(const MemReq& req) {
            if ((req.type == GETS) || (req.type == GETX)) {
                return true;                // see if the memory request need  to allocate 
            } else {
        ...
        uint64_t processEviction(const MemReq& triggerReq, Address wbLineAddr, int32_t lineId, uint64_t startCycle, bool* downgrade) {
            bool lowerLevelWriteback = false;
            uint64_t evCycle = tcc->processEviction(wbLineAddr, lineId, &lowerLevelWriteback, startCycle, triggerReq.srcId, downgrade); //1. if needed, send invalidates/downgrades to lower level
            evCycle = bcc->processEviction(wbLineAddr, lineId, lowerLevelWriteback, evCycle, triggerReq.srcId, downgrade); //2. if needed, write back line to upper level
            return evCycle;
        }
...
class MESITerminalCC : public CC {
    ...
    public:
        ...
        uint64_t processEviction(const MemReq& triggerReq, Address wbLineAddr, int32_t lineId, uint64_t startCycle, bool* downgrade) {
            bool lowerLevelWriteback = false;
            uint64_t endCycle = bcc->processEviction(wbLineAddr, lineId, lowerLevelWriteback, startCycle, triggerReq.srcId, downgrade); //2. if needed, write back line to upper level
