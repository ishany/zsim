...
uint64_t MESIBottomCC::processEviction(Address wbLineAddr, uint32_t lineId, bool lowerLevelWriteback, uint64_t cycle, uint32_t srcId, bool* downgrade) {
    //printf("L1 is processing eviction\n");
...
uint64_t MESITopCC::sendInvalidates(Address lineAddr, uint32_t lineId, InvType type, bool* reqWriteback, uint64_t cycle, uint32_t srcId, bool* downgrade) {
...
uint64_t MESITopCC::processEviction(Address wbLineAddr, uint32_t lineId, bool* reqWriteback, uint64_t cycle, uint32_t srcId, bool* downgrade) {
    //printf("L2 is processing eviction\n");
    if (nonInclusiveHack) {
        ...
        uint64_t result = sendInvalidates(wbLineAddr, lineId, INV, reqWriteback, cycle, srcId, downgrade);
        return result;
    }
}
...
uint64_t MESITopCC::processAccess(Address lineAddr, uint32_t lineId, AccessType type, uint32_t childId, bool haveExclusive,
    ...
    switch (type) {
            ...
            if (e->isEmpty() && haveExclusive && !(flags & MemReq::NOEXCL)) {
                ...
                *childState = E;
                ...
                if (e->isExclusive()) {
                    ...
                    respCycle = sendInvalidates(lineAddr, lineId, INVX, inducedWriteback, cycle, srcId, downgrade);
                }
        ...
        case GETX:
            ...
            respCycle = sendInvalidates(lineAddr, lineId, INV, inducedWriteback, cycle, srcId, downgrade);
...
uint64_t MESITopCC::processInval(Address lineAddr, uint32_t lineId, InvType type, bool* reqWriteback, uint64_t cycle, uint32_t srcId) {
    ...
    if (type == FWD) {//if it's a FWD, we should be inclusive for now, so we must have the line, just invLat works
        ...
        return sendInvalidates(lineAddr, lineId, type, reqWriteback, cycle, srcId, downgrade);
    }
}
